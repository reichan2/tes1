...
export patchtls=/rere-vmesswstls
export patchnontls=/rere-vmesswsntls
...
proxies:
  - {name: $user, server: ${domain}, port: $none, type: vmess, uuid: $uuid, alterId: 0, cipher: auto, tls: false, network: ws, ws-path: $patchnontls, ws-headers: {Host: ${domain}}}
proxy-groups:
  - name: ðŸš€ èŠ‚ç‚¹é€‰æ‹©
    type: select
    proxies:
...
external-controller: 127.0.0.1:9090
proxies:
  - {name: $user, server: ${domain}, port: $none, type: vmess, uuid: $uuid, alterId: 0, cipher: auto, tls: false, network: ws, ws-path: $patchnontls, ws-headers: {Host: bug.com}}
proxy-groups:
  - name: ðŸš€ èŠ‚ç‚¹é€‰æ‹©
    type: select
    proxies:
...
export patchtls=/rere-vlesswstls
export patchnontls=/rere-vlesswsntls
export uuid=$(cat /proc/sys/kernel/random/uuid)
read -p "   Expired (days) : " masaaktif
...
export vlesslink1="vless://${uuid}@${domain}:$tls?path=$patchtls&security=tls&encryption=none&type=ws&sni=${domain}#${user}"
export vlesslink2="vless://${uuid}@${domain}:$none?path=$patchnontls&encryption=none&host=${domain}&type=ws#${user}"
...
export vlesslink1="vless://${uuid}@${domain}:$xtls?security=xtls&encryption=none&headerType=none&type=tcp&flow=xtls-rprx-direct&sni=${domain}#${user}"
export vlesslink2="vless://${uuid}@${domain}:$xtls?security=xtls&encryption=none&headerType=none&type=tcp&flow=xtls-rprx-splice&sni=${domain}#${user}"
...
echo -e " [\e[36m•03\e[0m] Dellete Vmess        [\e[36m•08\e[0m] Dellete Vless"
echo -e " [\e[36m•04\e[0m] Renew Vmess          [\e[36m•09\e[0m] Renew Vless"
echo -e " [\e[36m•05\e[0m] Cek User Vmess       [\e[36m•10\e[0m] Cek User Vless"
echo -e ""
echo -e " [\e[36m•00\e[0m] Back To Menu         [\e[36m•xx\e[0m] Exit"
echo -e ""
